...
| `getResolution` | `getResolution` | `int getResolution(H3Index h);` | S2 | n/a | none (int return) | none | header line 513. Header doc: "Works on both cells and directed edges." **Parity hazard:** h3-js wraps this with `if (!H3.isValidCell(...)) return -1;` ("Compatability with stated API", h3core.js:787+). The design's "TS layer does exactly one thing" rule means we will *not* replicate that, so a differential test against h3-js will diverge on invalid input unless C++ adds the same guard. Flagging, not deciding. |
